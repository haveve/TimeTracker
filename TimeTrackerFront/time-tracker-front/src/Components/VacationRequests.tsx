...
export default function VacationRequests(props: { user: User }) {
  ...
  const renderStatus=(status:string)=>{
    switch(status){
      case "Approved": return (<p className="m-0 fs-5" style={{color: "green"}}>{status}</p>);
      case "Declined": return (<p className="m-0 fs-5" style={{color: "red"}}>{status}</p>);
      case "Canceled": return (<p className="m-0 fs-5" style={{color: "grey"}}>{status}</p>);
      case "Pending": return (<p className="m-0 fs-5" style={{color: "yellow"}}>{status}</p>);
      default: return (<></>);
    }
  }
  ...
  const renderReaction = (vacationRequest: VacationRequest, user:User) =>{
    ...
    return (<>
      ...
      <Button variant="outline-secondary" onClick={()=>HandleShowReactionModal(vacationRequest)}>Change reaction</Button>
    </>)
  }
                      ...
                      <Form onSubmit={(e) => HandleReactionModalSubmit(e)}>
                        ...
                        <Modal.Body>
                          <Form.Group>
                            ...
                            <Form.Select onChange={(e)=>HandleSetReaction(e.target.value)}>
