        ...
        <Container fluid className='p-0 m-0 h-100'>
            <Navbar expand={false} className="bg-black height-header">
                <Container fluid className='justify-content-start'>
                    ...
                    <Nav.Link as={Link} to={user ? "/User/" + user.login : "/Login"}
                              className='ms-auto'>{user ? user.login : "sign in"}</Nav.Link>
                    <Navbar.Offcanvas
                    ...
                    >
                            ...
                            <Nav className="justify-content-end flex-grow-1 pe-3">
                                <ListGroup>
                                    <ListGroup.Item action className='border-0 rounded-1 p-0 ps-2'>
                                        <Nav.Link as={Link} to={"/Users"} className='m-0'>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                 fill="currentColor" className="bi bi-people me-1 mb-1"
                                                 viewBox="0 0 16 16">
                                                <path
                                    ...
                                    <ListGroup.Item action className='border-0 rounded-1 p-0 ps-2'>
                                        <Nav.Link as={Link} to={"/CreateUser"} className='m-0'>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                        ...
                        <Nav className="justify-content-center ps-2 mb-3 flex flex-grow-1">
                            <ListGroup className="justify-content-end flex flex-grow-1 pe-3 ">
                                <ListGroup.Item action className='border-0 rounded-1 p-0 ps-2'><Nav.Link as={Link}
                                                                                                         to={"/Login"}
                                                                                                         className='m-0'
                                                                                                         onClick={() => {
            ...
            <NotificationModalWindow isShowed={errorTime !== ""} dropMassege={() => dispatch(clearErroMassageTime())}
                                     messegeType={MasssgeType.Error}>{errorTime}</NotificationModalWindow>
            <NotificationModalWindow isShowed={errorUserList !== ""}
                                     dropMassege={() => dispatch(clearErroMassageUserList())}
                                     messegeType={MasssgeType.Error}>{errorUserList}</NotificationModalWindow>
        </Container>
    );
}
