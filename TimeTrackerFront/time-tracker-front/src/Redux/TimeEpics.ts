...
import { RequestGetTime, RequestSetEndDate, RequestSetStartDate, GetWhetherTimerIsStarted } from "./Requests/TimeRequests";
...
export const setTimeE = (timeMark: TimeMark[], pageNumber: number, itemsInPage: number, offset: number, country: string) => ({
...
export const setIsStartedE = () => ({ type: "setIsStarted" })
export const setIsStartedEpic: Epic = (action$: Observable<PayloadAction<boolean>>) => {
    return action$.pipe(
        ofType("setIsStarted"),
        mergeMap(() => GetWhetherTimerIsStarted().pipe(
            map((isStarted) => changeTimerState(isStarted)),
            catchError(() => of(setErrorStatusAndErrorTime(ErrorMassagePattern)))
        ))
    )
}
